...
class MaxNetStreamPanel extends JPanel {
	public MaxNetStreamPanel() {
		...
		for (PointEllipse2D r : squares) {
			...
			if (r.linkEllipse2D1 != null) {
				...
				for (PointLink p : r.linkEllipse2D1) {
					...
					g2.drawString(((Integer) (p.maxCapcity)).toString() + "("
							+ p.currentCapcity + ")", tempX - SIDELENGTH / 2,
							tempY - SIDELENGTH / 2);
